...
import { getCityPlaces, PlacesQuery } from "@/lib/api";
import LiveCityTime from "@/components/LiveCityTime/LiveCityTime";
...
function Pagination({
  ...
  return (
    <nav
      ...
      aria-label="City places pagination"
    >
      {page > 1 && (
        <Link
          href={buildQuery(id, page - 1, query)}
          className="rounded-full border border-[var(--border)] px-4 py-2 text-sm font-semibold text-[var(--foreground)] transition-colors hover:border-[var(--primary)] hover:text-[var(--primary)]"
        >
          Previous
        </Link>
      )}
      ...
      {pages.map((pageNumber) => (
        <Link
          key={pageNumber}
          href={buildQuery(id, pageNumber, query)}
          aria-current={
      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 pt-8 sm:px-6 sm:pt-12 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-3 max-w-2xl text-base leading-7 text-[var(--muted)]">
            Explore places, restaurants, attractions and
            other points of interest in this city.
          </p>

          <div className="mt-5 max-w-xl">
            <CityWeather cityId={result.city.id} />
          </div>
        </div>
        ...
        <section className="mb-10">
          <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
            ...
            <Link
              href={`/cities/${result.city.id}/digital-twin`}
        ...
        <form
          className="mb-10 grid min-w-0 gap-3 rounded-3xl border border-[var(--border)] bg-[var(--card)] p-4 sm:p-5 md:grid-cols-4"
          method="get"
        >
          <input
            name="search"
          ...
          >
            ...
            {Array.from(new Set(subtypes)).map(
              (subtype) => (
                <option
                  key={subtype}
                  value={subtype}
          ...
          <div className="flex min-w-0 flex-col gap-3 sm:flex-row">
            <select
              ...
              className="min-w-0 flex-1 rounded-2xl border border-[var(--border)] bg-transparent px-4 py-3 text-sm outline-none"
            >
            ...
            <button
              type="submit"
              className="w-full shrink-0 rounded-2xl bg-[var(--primary)] px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-[var(--primary-hover)] sm:w-auto"
            >
              Apply
            </button>
          </div>
